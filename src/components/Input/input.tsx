 ...
 */
 ...
 export interface inputRefProps {
    setJudgeState: (v: string) => void,
    getJudgeState: () => void,
    getValue: () => void,
    getDesc: () => void
 }

 export const Input = forwardRef<inputRefProps, InputProps>((props, ref) => {
        let {
            title,
            className,
            type,
    ...
    const [judgeState, setJudgeState] = useState(JUDGE_STATE.STATIC)
    ...
    const timeSign = useRef<string>(`${new Date().getTime()}`)
        ...
        useEffect(() => {
            setTimeout(() => setValue(() => initValue), 1000)
        }, [])
