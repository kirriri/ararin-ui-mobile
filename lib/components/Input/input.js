...
export var Input = forwardRef(function (props, ref) {
    var _a;
    var title = props.title, className = props.className, type = props.type, judge = props.judge, onClick = props.onClick, codeTxt = props.codeTxt, times = props.times, onBlur = props.onBlur, onFocus = props.onFocus, noHide = props.noHide, noJudge = props.noJudge, initValue = props.initValue, restPros = __rest(props, ["title", "className", "type", "judge", "onClick", "codeTxt", "times", "onBlur", "onFocus", "noHide", "noJudge", "initValue"]);
    var _b = useState(true), firstInput = _b[0], setFirstInput = _b[1];
    var _c = useState(''), desc = _c[0], setDesc = _c[1];
    var _d = useState(JUDE_STATE.STATIC), judgeState = _d[0], setJudgeState = _d[1];
    ...
    useEffect(function () {
        initValue && setValue(function () { return initValue; });
    }, []);
    var classes = classNames('ararin-input', className, (_a = {},
        _a["ararin-input-" + type] = type,
        _a['failed'] = (judgeState === JUDE_STATE.FAILED && !firstInput),
        _a));
            ...
            case "mobilePhone":
                if ((/^1[0-9]{0,}$/.test(text)) || text === "") {
                    if (text.length <= 11) {
                        ...
                        if (text.length === 11) {
                            setJudgeState(function () { return JUDE_STATE.SUCCESS; });
                        }
                        else {
                            setJudgeState(function () { return JUDE_STATE.FAILED; });
                        }
                    }
                }
                return;
        ...
        React.createElement("div", { className: "ararin-input-zone" },
            type === 'input' &&
                React.createElement(React.Fragment, null,
                    React.createElement("input", __assign({ type: noHide ? '' : (hideJudge.current.indexOf(judge) !== -1 && showHide) ? 'password' : '', value: value, onChange: handleInput, onFocus: function (e) {
                            onFocus && onFocus(e);
                            // setFirstInput(() => false)
                        }, onBlur: function (e) {
                            onBlur && onBlur(e);
