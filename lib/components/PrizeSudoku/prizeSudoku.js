...
export var PrizeSudoku = function (props) {
    ...
    var handleLotteryClick = function (e) {
        ...
        if (data.length === 9 && onClick && !state.wheeling) {
            ...
            onClick().then(function (award) {
                if (award.flag) {
                    var awardIndex_1 = parseInt(keyMap.current[award.index]);
                    if (awardIndex_1 + '' !== 'NaN') {
                        setState(function (props) { return (__assign(__assign({}, props), { awardIndex: awardIndex_1 })); });
    ...
    var roll = function (awardIndex) {
        ...
        if ((sudoku.current.times > sudoku.current.cycle + 10) && awardIndex === sudoku.current.index) {
            ...
            animateTimer.current = setTimeout(function () { return roll(awardIndex); }, sudoku.current.speed);
        }
        return false;
    };
    return React.createElement("div", __assign({}, restProps, { className: classes + " ararin-prizeSudoku-wrapper" }),
        React.createElement("div", { className: "ararin-prizeSudoku-box" },
            React.createElement("div", { className: "ararin-prizeSudoku-zone" },
                React.createElement("ul", { className: "ararin-clear-fix" }, renderData.map(function (item, index) {
                    return index != 4
                        ? React.createElement("li", { className: "\n                                            arain-prizeSudoku-prize \n                                            prize-" + keyMap.current[index] + " \n                                            " + (state.index == keyMap.current[index] ? 'active' : ''), key: "arain-prizeSudoku-prize arain-prizeSudoku-prize" + index },
